...
public class Parts_BummerEffect : MonoBehaviour
{
    // 페이드아웃시키는 부분
    ...
    void Update()
    {
        ...
        if (StartEffect > 0.3f) { ChangeDir_Part3(ChangeDir_Check[1]); }
        if (StartEffect > 0.7f) { ChangeDir_Part2(ChangeDir_Check[2]); }
    }

    private void MovingPartBody1_LR(Vector3 dir1)
    {
    ...
    private void MovingPartBody2_LR(Vector3 dir2)
    {
    ...
    private void MovingPartBody3_LR(Vector3 dir3)
    {
    ...
    void ChangeDir_Part1(bool dir1)
    {
        if(dir1) { MovingPartBody1_LR(MoveDir_Part1[0]); }
        else { MovingPartBody1_LR(MoveDir_Part1[1]); }
    }

    void ChangeDir_Part2(bool dir2) 
    {
        if (dir2) { MovingPartBody2_LR(MoveDir_Part2[0]); }
        else { MovingPartBody2_LR(MoveDir_Part2[1]); }
    }

    void ChangeDir_Part3(bool dir3)
    {
        if (dir3) { MovingPartBody3_LR(MoveDir_Part3[0]); }
