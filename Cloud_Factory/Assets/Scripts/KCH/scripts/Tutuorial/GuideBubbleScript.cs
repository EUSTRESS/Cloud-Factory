...
public class GuideBubbleScript : MonoBehaviour
{
	...
	public void LoadText()
	{
		if (LanguageManager.GetInstance().GetCurrentLanguage() == "Korean")
		{
			...
			mDialog[8, 2] = "구름을 받고 감정에 변화가 생긴 손님은 집으로 돌아갑니다. 조만간 다시 방문할지도 모르겠네요!";
			mDialog[8, 3] = "FadeOut81";
			mDialog[8, 4] = "클라우드 팩토리의 안내를 이것으로 마칩니다. 여러 뭉티의 안식처가 될 수 있는 클라우드 팩토리가 될 수 있으면 좋겠습니다. 행운을 빕니다!";
			mDialog[8, 5] = "End";
		}
		else
		{
	...
	public void ProcessSpecialText()
	{
		...
		if (mDialog[mDialogIndex, currentDialogNum] == "FadeOut73")
		{
			...
			currentDialogNum++;
			mTutorialManager.SetActiveFadeOutScreen(false);
			mTutorialManager.FadeOutCloudStorageSend();
			this.gameObject.transform.SetAsLastSibling();
		}
